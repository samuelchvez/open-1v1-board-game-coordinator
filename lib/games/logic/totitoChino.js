...
TotitoChino.prototype.getStartingBoard = function(){
  ...
  if(playingTurnID === game.currentTurn){
    ...
    if(positionsToFill.length > 0){
      ...
      for(var i = 0; i < game.board[0].length; i++){
          if(((i + 1) % this.N) !== 0){
              if(game.board[0][x] !== EMPTY && game.board[0][x + 1] !== EMPTY && game.board[1][contador + acumulador] !== EMPTY && game.board[1][contador + acumulador + 1] !== EMPTY){
