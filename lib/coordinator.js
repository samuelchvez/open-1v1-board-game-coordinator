...
tournaments[tournamentID] = new League(tournamentID, roundRobinTimes, game);

function getRoom(tournament) {
  return "tournament:" + tournament.id;
}

function getViewerRoom(tournament) {
  return "tournament viewers:" + tournament.id;
}
...
function signIn(socket, data) {
  ...
  if (tournamentID in tournaments) {
    ...
    // Generate tournament viewer room
    viewerRoom = getViewerRoom(tournament);
    ...
    if (role === gameConstants.INGAME_USER_ROLES.player) {
      ...
      tournament.registerPlayer(
        ...
        function (response) {
          // Log arrival
          console.log(
            "New player registered to the tournament: " + player.user_name
          );
        ...
        function (response) {
          // Log rejection
          console.log(
            "New player rejected from the tournament " + tournamentID
          );
    ...
    } else if (role === gameConstants.INGAME_USER_ROLES.admin) {
      // Join socket to the tournament room
      socket.join(room);

      // Emit success signin signal
      socket.emit("ok_signin", { game: gameType });
    ...
    } else if (role === gameConstants.INGAME_USER_ROLES.viewer) {
      // Fetch viewer
      // TODO: Unmock the ranking and id definition
      viewer = new Viewer(gameRandom.getID(), data.user_name, socket);

      tournament.registerViewer(
        viewer,
        ...
        function (response) {
          ...
          // Join socket to the tournament viewers room
          socket.join(viewerRoom);

          // Register viewer / tournament relation, used in disconnect event
          (function (s, r, t, v) {
            s.on("disconnect", function () {
              // Socket leaving room
              s.leave(r);

              // Unregister viewer from tournament
              t.unregisterViewer(
                v,
                function (_data) {},
        ...
        function (response) {
          // Log rejection
          console.log(
            "New viewer rejected from the tournament " + tournamentID
          );
...
function nextGameDaemonProcedure(socket, tournament) {
  ...
  if (!tournament.finished()) {
    ...
    socket.broadcast.to(roomName).emit("tournament_status_changed", {
      status: tournamentConstants.STATUS.finished,
    });

    socket.emit("tournament_status_changed", {
      status: tournamentConstants.STATUS.finished,
    });
  }
}
  ...
  if (tournamentID in tournaments) {
    ...
    if (tournament.status === tournamentConstants.STATUS.waiting) {
      ...
      tournament.start(function (games) {
        ...
        socket.broadcast
          .to(getRoom(tournament))
          .emit("tournament_status_changed", {
            status: tournamentConstants.STATUS.ongoing,
          });
        socket.emit("tournament_status_changed", {
          status: tournamentConstants.STATUS.ongoing,
        });
        ...
        // Start daemon
        tournament.daemonIntervalID = setInterval(function () {
...
function playerReady(socket, data) {
  ...
  if (tournamentID in tournaments) {
    ...
    if (gameID in tournament.ongoingGames) {
      ...
      // Select player
      player =
        turnID === gameConstants.PLAYER_1_TURN_ID
          ? game.player_1
          : game.player_2;
...
function play(socket, data) {
  ...
  if (tournamentID in tournaments) {
    var tournament = tournaments[tournamentID],
      roomName = getRoom(tournament),
      viwerRoomName = getViewerRoom(tournament);

    // If the game is in the tournament
    if (gameID in tournament.ongoingGames) {
      var game = tournament.ongoingGames[gameID],
        player =
          playerTurnID === gameConstants.PLAYER_1_TURN_ID
            ? game.player_1
            : game.player_2;
      (player_1 = game.player_1), (player_2 = game.player_2);
      ...
      game.play(
        ...
        function (cGame, winnerPlayer) {
          ...
          if (winnerPlayer)
            winnerTurnID =
              cGame.player_1.id === winnerPlayer.id
          ...
          // Notify game finished to the viewers
          socket.broadcast.to(viwerRoomName).emit("game_finished", {
            game_id: cGame.id,
            movement_log: cGame.movement_log,
            winner_turn_id: winnerTurnID,
            player_1: {
              id: cGame.player_1.id,
              user_name: cGame.player_1.user_name,
              wins: cGame.player_1.wins,
              loses: cGame.player_1.loses,
              draws: cGame.player_1.draws,
            },
            player_2: {
              id: cGame.player_2.id,
              user_name: cGame.player_2.user_name,
              wins: cGame.player_2.wins,
              loses: cGame.player_2.loses,
              draws: cGame.player_2.draws,
            },
          });

          // Remove movement log to save memory
          cGame.flushmovement_log();
    ...
    if (tournament.status !== tournamentConstants.STATUS.waiting) {
      ...
      tournament.reset(function () {
        ...
        socket.broadcast
          .to(getRoom(tournament))
          .emit("tournament_status_changed", {
            status: tournamentConstants.STATUS.waiting,
          });
        socket.emit("tournament_status_changed", {
          status: tournamentConstants.STATUS.waiting,
        });
        ...
        console.log("");
        console.log(
          "ATTENTION!! Tournament " + tournament.id + " has been reset"
        );
        console.log("");
